...
// 상수 및 설정
// ----------------------------------------------------------------------
const APP_VERSION = "v0.5.6-Fix"; // Full Code Restoration & Features
const BUILD_DATE = "2024.06.21";
...
export default function App() {
  ...
  useEffect(() => {
    ...
    style.innerHTML = `
      @media print {
        ...
        .print-area { position: absolute; left: 0; top: 0; width: 100%; margin: 0; padding: 20px; background: white; color: black; z-index: 9999; }
  ...
  // This updates the product's 'spaces' array (Reverse Link)
  const handleSpaceProductToggle = async (spaceId, productId, isAdded) => { 
     ...
     if (isFirebaseAvailable && db) { await setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'products', product.id), { spaces: newSpaces }, { merge: true }); } 
     else { const idx = products.findIndex(p => p.id === productId); const newProds = [...products]; newProds[idx] = { ...product, spaces: newSpaces }; saveToLocalStorage(newProds); } 
  };
...
function ProductCard({ product, onClick, showMoveControls, onMove, isFavorite, onToggleFavorite }) {
  ...
  const awardBadge = product.awards?.[0]; // Award Only per request
  return (
...
function ProductFormModal({ categories, existingData, onClose, onSave, onDelete, isFirebaseAvailable, initialCategory }) {
  ...
  useEffect(() => {
    if (existingData) {
      setFormData({ 
        ...
        optionsString: existingData.options?.join(', ') || '',
        materialsString: existingData.materials?.join(', ') || '',
        bodyColorsString: existingData.bodyColors?.join(', ') || '',
        upholsteryColorsString: existingData.upholsteryColors?.join(', ') || '',
        awardsString: existingData.awards?.join(', ') || '',
        ...
        contentImages: existingData.contentImages || [],
  ...
  const handleImageUpload = async (e) => {
    ...
    if (files.length > 0) {
      ...
      const newUrls = [];
      for (const file of files) { try { newUrls.push(await processImage(file)); } catch (e) {} }
  ...
  const handleContentImageUpload = async (e) => {
    ...
    if (files.length > 0) {
      ...
      const newUrls = [];
      for (const file of files) { try { newUrls.push(await processImage(file)); } catch (e) {} }
      setFormData(prev => ({ ...prev, contentImages: [...prev.contentImages, ...newUrls] }));
      setIsProcessingImage(false);
    }
  };
  ...
  const handleAttachmentUpload = (e) => {
    ...
    files.forEach(file => {
      ...
      const reader = new FileReader();
      reader.onload = (e) => setFormData(p => ({...p, attachments: [...p.attachments, {name: file.name, url: e.target.result}]}));
      reader.readAsDataURL(file);
    });
  };
  ...
  const handleAddLinkAttachment = () => {
    ...
    if (url && name) setFormData(p => ({...p, attachments: [...p.attachments, {name, url}]}));
  };
      ...
      <div className="bg-white w-full max-w-3xl rounded-2xl overflow-hidden shadow-2xl flex flex-col max-h-[95vh] animate-in slide-in-from-bottom-4 duration-200">
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-8 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-6">
             ...
             <div><label className="block text-xs font-bold text-zinc-500 uppercase mb-1">Awards (comma)</label><input className="w-full border p-2 rounded-lg" value={formData.awardsString} onChange={e=>setFormData({...formData, awardsString: e.target.value})}/></div>
          </div>
          <div className="grid grid-cols-2 gap-6 bg-zinc-50 p-4 rounded-xl border border-zinc-100">
             <div><label className="block text-xs font-bold text-zinc-500 uppercase mb-1">Body Colors</label><input className="w-full border p-2 rounded-lg" placeholder="Black, #fff..." value={formData.bodyColorsString} onChange={e=>setFormData({...formData, bodyColorsString: e.target.value})}/></div>
             <div><label className="block text-xs font-bold text-zinc-500 uppercase mb-1">Upholstery Colors</label><input className="w-full border p-2 rounded-lg" placeholder="Red, Blue..." value={formData.upholsteryColorsString} onChange={e=>setFormData({...formData, upholsteryColorsString: e.target.value})}/></div>
          </div>
          <div className="space-y-4">
             <div><label className="block text-xs font-bold text-zinc-500 uppercase mb-1">Link</label><input className="w-full border p-2 rounded-lg" placeholder="https://..." value={formData.productLink} onChange={e=>setFormData({...formData, productLink: e.target.value})}/></div>
             <div className="flex justify-between"><label className="block text-xs font-bold text-zinc-500 uppercase">Attachments</label><div className="space-x-2"><button type="button" onClick={()=>document.getElementById('doc-upload').click()} className="text-xs border px-2 py-1 rounded bg-white">File</button><button type="button" onClick={handleAddLinkAttachment} className="text-xs border px-2 py-1 rounded bg-white">Link</button></div><input id="doc-upload" type="file" multiple className="hidden" onChange={handleAttachmentUpload}/></div>
             <div className="space-y-1">{formData.attachments.map((f, i)=><div key={i} className="flex justify-between text-xs bg-zinc-50 p-2 rounded"><span>{f.name}</span><button type="button" onClick={()=>removeAttachment(i)} className="text-red-500"><X className="w-3 h-3"/></button></div>)}</div>
          </div>

...
export default function App() {
  ...
  // V 0.8.73: Admin Move Item (Button based)
  const handleMoveItem = async (collectionName, items, index, direction, parentId = null) => {
      ...
      // Optimistic Update
      if (collectionName === 'products') setProducts(newItems);
      else if (collectionName === 'swatches') setSwatches(newItems);
      else if (collectionName === 'awards') setAwards(newItems);
      else if (collectionName === 'scenes' && parentId) {
           const newContent = { ...spaceContents[parentId], scenes: newItems };
           setSpaceContents(prev => ({ ...prev, [parentId]: newContent }));
      }
      
      await handleBatchReorder(collectionName, newItems, parentId);
  };
